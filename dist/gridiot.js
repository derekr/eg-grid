...
function attachPointer(core) {
	...
	const onPointerDown = (e) => {
		const item = e.target.closest("[data-gridiot-item]");
		if (!item) return;
		core.select(item);
		e.preventDefault();
...
function attachKeyboard(core) {
	...
	const findItemInDirection = (fromCell, direction, excludeItem) => {
		const items = Array.from(core.element.querySelectorAll("[data-gridiot-item]"));
		let bestItem = null;
	...
	const onKeyDown = (e) => {
		if (e.key === "G" && e.shiftKey && !e.ctrlKey && !e.altKey && !e.metaKey) {
			...
			} else core.element.removeAttribute("data-gridiot-keyboard-mode");
			return;
		}
...
function attachAccessibility(core) {
	...
	function resolveTemplate(item, event, vars, fallback) {
		const template = item.getAttribute(`data-gridiot-announce-${event}`) || core.element.getAttribute(`data-gridiot-announce-${event}`);
...
function attachResize(gridElement, options) {
	...
	function emit(event, detail) {
		gridElement.dispatchEvent(new CustomEvent(`gridiot:${event}`, {
			bubbles: true,
			detail
		}));
	}
	...
	const onPointerMove = (e) => {
		...
		if (item) {
			...
			if (handle !== hoveredHandle || item !== hoveredItem) {
				...
				if (hoveredItem === item && hoveredHandle && !handle) item.removeAttribute("data-gridiot-handle-hover");
				hoveredItem = item;
				hoveredHandle = handle;
				item.style.cursor = (handle ? CURSOR[handle] : "") || "";
				if (handle) item.setAttribute("data-gridiot-handle-hover", handle);
				else item.removeAttribute("data-gridiot-handle-hover");
			}
		} else if (hoveredItem) {
			hoveredItem.style.cursor = "";
			hoveredItem.removeAttribute("data-gridiot-handle-hover");
	...
	function setScrolling(active) {
		if (active) {
			...
			settleTimeoutId = setTimeout(() => {
				if (core) core.cameraScrolling = false;
				settleTimeoutId = null;
				gridElement.dispatchEvent(new CustomEvent("gridiot:camera-settled", { bubbles: true }));
			}, settleDelay);
		}
	}
...
function attachPlaceholder(gridElement, options = {}) {
	const { className = "gridiot-placeholder", element: customElement, disableViewTransition = true } = options;
	...
	function handlePointerUp() {
		requestAnimationFrame(() => {
			if (placeholder && !document.querySelector("[data-gridiot-dragging]") && !document.querySelector("[data-gridiot-resizing]")) remove();
		});
	}
	...
	const removeGridListeners = listenEvents(gridElement, {
		"gridiot:drag-start": handleDragStart,
		"gridiot:drag-move": handleDragMove,
		"gridiot:drag-end": handleDragEnd,
		"gridiot:drag-cancel": handleDragCancel,
		"gridiot:drop-preview": handleDropPreview,
		"gridiot:resize-start": handleResizeStart,
		"gridiot:resize-move": handleResizeMove,
		"gridiot:resize-end": handleResizeEnd,
		"gridiot:resize-cancel": handleResizeCancel
	});
...
function layoutToCSS(items, options = {}) {
	const { selectorPrefix = "#", selectorSuffix = "", excludeSelector = "", maxColumns } = options;
	const rules = [];
	for (const item of items) {
...
function readItemsFromDOM(container) {
	...
	return Array.from(elements).map((el) => {
		...
		return {
			id: element.dataset.id || element.dataset.gridiotItem || "",
			column,
			row,
			width,
			height
...
function attachResponsive(gridElement, options, core) {
	...
	const resizeObserver = new ResizeObserver(() => {
		...
		if (newColumnCount !== lastColumnCount) {
			...
			gridElement.dispatchEvent(new CustomEvent("gridiot:column-count-change", {
				bubbles: true,
				detail: {
					previousCount,
...
function init(element, options = {}) {
	...
	const core = {
		...
		select(item) {
			...
			if (previousItem) previousItem.removeAttribute("data-gridiot-selected");
			if (item) {
				const itemId = item.id || item.getAttribute("data-gridiot-item") || "";
				stateMachine.transition({
					type: "SELECT",
		...
		emit(event, detail) {
			element.dispatchEvent(new CustomEvent(`gridiot:${event}`, {
				bubbles: true,
				detail
			}));
		},
...
function createLayoutModel(options) {
	...
	return {
		...
		generateAllBreakpointCSS(options) {
			const { selectorPrefix = "#", selectorSuffix = "", cellSize, gap, gridSelector = ".grid-container" } = options ?? {
				cellSize: 184,
				gap: 16
			};
			const cssRules = [];
